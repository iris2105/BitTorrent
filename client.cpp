...
int main(int argc, char* argv[]) {
    if (argc < 3) {
        cerr << "Usage: " << argv[0] << " <peerIP:port> <tracker_info.txt>" << endl;
        return 1;
    }

    // Split peer IP and port
    vector<string> peerString = makeSplit(argv[1], ':');
    if (peerString.size() != 2) {
        cerr << "Error: Invalid format for peer IP:port. Use format like 127.0.0.1:8000" << endl;
        return 1;
    }
    ...
    try {
        ...
        cerr << "Error: Invalid port number for peer" << endl;
        return 1;
    }
    ...
    if (!f.is_open()) {
        cerr << "Error: Could not open tracker info file: " << trackerInfoFile << endl;
        return 1;
    }
    ...
    string port_str;
    f >> ipinp >> port_str;
    if (ipinp.empty() || port_str.empty()) {
        cerr << "Error: tracker_info.txt is empty or malformed. Expected: <IP> <PORT>" << endl;
        return 1;
    }

    try {
        trackerPort = stoi(port_str);
    } catch (...) {
        cerr << "Error: Invalid port number in tracker_info.txt" << endl;
        return 1;
    }
    ...
    if (inet_pton(AF_INET, trackerIP, &addr.sin_addr) <= 0) {
